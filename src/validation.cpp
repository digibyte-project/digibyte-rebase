...
static void UpdateMempoolForReorg(CTxMemPool& mempool, CTxMemPool& stempool, DisconnectedBlockTransactions& disconnectpool, bool fAddToMempool) EXCLUSIVE_LOCKS_REQUIRED(cs_main, mempool.cs, stempool.cs)
{
    ...
    while (it != disconnectpool.queuedTx.get<insertion_order>().rend()) {
        ...
        if (!fAddToMempool || (*it)->IsCoinBase() ||
            !AcceptToMemoryPool(mempool, stempool, stateDummy, *it,
...
static bool AcceptToMemoryPoolWithTime(const CChainParams& chainparams, CTxMemPool& pool, CTxMemPool& s_pool, TxValidationState &state,
                        const CTransactionRef &tx, int64_t nAcceptTime, std::list<CTransactionRef>* plTxnReplaced,
...
bool AcceptToMemoryPool(CTxMemPool& pool, CTxMemPool& s_pool, TxValidationState &state, const CTransactionRef &tx,
                        std::list<CTransactionRef>* plTxnReplaced, bool bypass_limits, bool test_accept, CAmount* fee_out)
{
    const CChainParams& chainparams = Params();
    return AcceptToMemoryPoolWithTime(chainparams, pool, s_pool, state, tx, GetTime(), plTxnReplaced, bypass_limits, test_accept, fee_out);
}
...
static void UpdateTip(CTxMemPool& mempool, CTxMemPool& stempool, const CBlockIndex* pindexNew, const CChainParams& chainParams)
