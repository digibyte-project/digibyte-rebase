...
public:
    ...
    // Last dandelion relay
    int64_t lastDandelionActivity{0};
...
public:
    ...
    //! this section requires modification, as once our tx becomes a uint256& reference,
    //! we no longer know the original CInv type that it carried (barrystyle 21012021).

    void AddKnownTx(const CInv& inv)
    {
        if (m_tx_relay != nullptr) {
            ...
            if (inv.type == MSG_TX || inv.type == MSG_WTX) {
                ...
                m_tx_relay->setDandelionInventoryKnown.insert(inv.hash);
            }
        }
    }

    void PushTxInventory(const CInv& inv)
    {
        if (m_tx_relay == nullptr) return;
        LOCK(m_tx_relay->cs_tx_inventory);
        if (!m_tx_relay->filterInventoryKnown.contains(inv.hash)) {
            m_tx_relay->setInventoryTxToSend.insert(inv.hash);
        } else if (!m_tx_relay->setDandelionInventoryKnown.count(inv.hash)) {
